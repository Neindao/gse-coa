...
function GSE.GUIShowViewer()
  ...
  for k,v in GSE.pairsByKeys(names) do
    ...
    local tclassid = GSE.ResolveClassKey(elements[1])
    if tclassid ~= cclassid then
      ...
      sectionheader:SetText(Statics.wotlkSpecIDList[cclassid] or tostring(cclassid))
...
function GSE.GUIConfigureMacroButton(button)
  if GSE.OOCCheckMacroCreated(GSE.GUIViewFrame.SequenceName) then
    ...
    button:SetCallback("OnClick", function()
      ...
      GSE.GUIViewFrame.panels[viewframe.ClassID .."," .. GSE.GUIViewFrame.SequenceName].Icon:SetImage(GSE.GetMacroIcon(GSE.ResolveClassKey(viewframe.ClassID), GSE.GUIViewFrame.SequenceName))
    end)
  else
