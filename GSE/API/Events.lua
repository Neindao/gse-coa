...
function GSE:ADDON_LOADED(event, addon)
  ...
  if addon == GNOME then
    ...
    if not GSEOptions.HideLoginMessage then
      ...
      GSE.Print("New: Type " .. GSEOptions.CommandColour .. "/gse loadsamples|r to load sample macros for your class.", GNOME)
    end
  end
...
function GSE:GSSlash(input)
  ...
  elseif string.lower(input) == "coa" then
    local info = GSE.GetCoAEnvironmentInfo and GSE.GetCoAEnvironmentInfo() or {}
    GSE.Print("===== GSE-CoA Diagnostics =====", "CoA")
    GSE.Print("Environment: " .. tostring(info.environment), "CoA")
    GSE.Print("Class: " .. tostring(info.className) .. " / " .. tostring(info.classFile), "CoA")
    GSE.Print("GSE ClassID: " .. tostring(info.classID), "CoA")
    GSE.Print("Spec/Profile: " .. tostring(info.specID) .. " / " .. tostring(info.storageProfile), "CoA")
    GSE.Print("ClassOnlyMode: " .. tostring(info.classOnlyMode), "CoA")
    GSE.Print("LastSpecFailure: " .. tostring(info.lastSpecFailure), "CoA")
    GSE.Print("C_ClassInfo: " .. tostring(info.hasCClassInfo) .. " | C_ClassTalents: " .. tostring(info.hasCClassTalents) .. " | C_Traits: " .. tostring(info.hasCTraits), "CoA")
    GSE.Print("IsCustomClass: " .. tostring(info.isCustomClass) .. " | BuildDraft: " .. tostring(info.buildDraft), "CoA")
    GSE.Print("===============================", "CoA")
  elseif string.lower(string.sub(input,1,6)) == "coaseq" then
    local seqname = string.sub(input,8)
    if GSE.CoADumpSequence then
      GSE.CoADumpSequence(seqname)
    else
      GSE.Print("GSE-CoA: Sequence diagnostics are unavailable in this build.", "CoA")
    end
  ...
  elseif string.lower(input) == "loadsamples" then
    if GSE.LoadDocumentedSampleMacros then
      GSE.LoadDocumentedSampleMacros()
      GSE.Print("Sample macros for your class have been loaded. Type /gse to view them.", GNOME)
    else
      GSE.Print("Sample macros are not available.", GNOME)
    end
  else
    GSE.GUIShowViewer()
  end
...
function GSE:processReload(action, arg)
  if arg == "Samples" then
    ...
    GSE.Print("The Sample Macros have been reloaded.")
  end
end
...
function GSE.prepareTooltipOOCLine(tooltip, OOCEvent, row, oockey)
  tooltip:SetCell(row, 1, tostring(OOCEvent.action), "LEFT", 1)
